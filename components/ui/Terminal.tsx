...
export default function Terminal() {
  ...
  const sendEmail = (e: React.FormEvent) => {
    ...
    const TEMPLATE_ID = 'template_h4tsst4'; // Ensure you replace this with your actual Template ID
  ...
  const socialLinks = [
    ...
    { name: "LINKEDIN", icon: Linkedin, href: "https://www.linkedin.com/in/sambhav-jaiswal-4b13a4289", label: "sambhav-jaiswal" },
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
            {socialLinks.map((link) => (
              <a 
                ...
                className="flex items-center gap-3 p-3 border border-zinc-900 bg-black hover:border-red-600 transition-all group"
              >
          ...
          {status === 'idle' ? (
            <form ref={form} onSubmit={sendEmail} className="space-y-8">
              <div className="flex items-center gap-2 text-zinc-500 text-xs italic">
                ...
                <span>Initializing secure transmission to operator...</span>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input 
                    required 
                    name="user_name" 
                    className="bg-transparent border-b border-zinc-900 focus:border-red-600 outline-none text-white text-xs w-full py-2 transition-colors uppercase" 
                    placeholder="_" 
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    name="message" 
                    rows={3} 
                    className="bg-zinc-900/50 p-4 border border-zinc-800 focus:border-red-600 outline-none text-white text-xs w-full resize-none transition-colors uppercase" 
              ...
              <button 
                ...
                className="w-full py-4 bg-red-600 hover:bg-white text-white hover:text-black transition-all font-black text-[10px] tracking-[0.4em] uppercase disabled:opacity-50"
              >
            ...
            >
              ...
              <p className="text-zinc-600 text-[9px] mt-2 tracking-widest uppercase">Operator will review logs shortly.</p>
              <button 
                onClick={() => setStatus('idle')} 
                className="text-red-600 text-[9px] mt-8 hover:underline font-black uppercase tracking-widest"
              >
                {/* FIXED: Escaped the '>' character */}
            ...
            <div className="py-10 text-center text-red-600 border border-red-600/20 bg-red-600/5 px-4">
              ...
              <p className="text-zinc-600 text-[9px] mt-2 tracking-widest uppercase italic">Check network diagnostics and retry.</p>
              <button 
                onClick={() => setStatus('idle')} 
                className="text-white text-[9px] mt-8 hover:underline uppercase font-black tracking-widest"
              >
                {/* FIXED: Escaped the '>' character */}
                {'>'} Retry_Handshake
              </button>
